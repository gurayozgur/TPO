"""LoRA layers for CLIP multi-head attention.

Vendored, unmodified in behaviour, from FoundPAD [1], which in turn reconstructs
loralib [2] (Microsoft, MIT licence) following CLIP-LoRA [3]. Only the classes the
FoundPAD detector actually uses are kept: `LoRALayer`, `LinearLoRA` and
`PlainMultiheadAttentionLoRA`.

Note the rank-stabilised scaling `alpha / sqrt(r)` (rsLoRA) in `LoRALayer`, which
is what FoundPAD uses and what the released TPO checkpoint was trained with.

[1] Ozgur et al., "FoundPAD: Foundation Models Reloaded for Face Presentation
    Attack Detection", WACVW 2025. https://github.com/gurayozgur/FoundPAD
[2] https://github.com/microsoft/LoRA
[3] Zanella and Ben Ayed, "Low-Rank Few-Shot Adaptation of Vision-Language
    Models", CVPRW 2024.
"""
